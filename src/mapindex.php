...
<div id="mobile-sidebar" style="display: none;">
    <button id="close-menu-btn">× 閉じる</button>
    <ul id="mobile-friend-list"></ul>
    <button id="mobile-update-location-btn" style="display: none;">位置情報を更新</button>
</div>
...
// フォローしているユーザーのデータ
const followedUsers = <?php echo json_encode($followedUsers); ?>;
const mobileFriendList = document.getElementById('mobile-friend-list');
...
followedUsers.forEach(user => {
    if (user.icon_name && user.user_name) {
        ...
        listItem.addEventListener('click', () => {
            const userPosition = [user.longitude, user.latitude];
            mobileMap.flyTo({ center: userPosition, zoom: 15 });

            showPopup(userPosition, user.user_name); // ポップアップ表示
            document.getElementById('mobile-sidebar').style.display = 'none'; // メニューを閉じる
        });

        mobileFriendList.appendChild(listItem);
    }
});

// マーカーの作成関数
function addMarker(user, iconUrl, position) {
    const markerElement = document.createElement('div');
    markerElement.className = 'marker';
...
// 現在地を取得し、マーカーを表示する（ボタンを最初は非表示）
document.getElementById('mobile-update-location-btn').style.display = 'none';

// フォローしているユーザーの位置にマーカーを表示
followedUsers.forEach(user => {
    if (user.icon_name && user.latitude && user.longitude) {
        addMarker(user, user.icon_name, [user.longitude, user.latitude]);
    }
});

// ポップアップ表示用関数
function showPopup(position, userName) {
    new mapboxgl.Popup()
        .setLngLat(position)
        .setHTML(`<div>ユーザー名: ${userName}</div>`)
        .addTo(mobileMap);
}
</script>
