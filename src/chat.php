<f?php require 'parts/auto-login.php' ; try { $pdo=new PDO("mysql:host=" . SERVER . " ;dbname=" . DBNAME, USER, PASS);
...
if ($partner_id === null || $logged_in_user_id === null) {
    echo " ユーザーIDが指定されていません。"; exit(); } // メッセージを取得し既読フラグを更新する関数 function getMessages($pdo, $logged_in_user_id,
    $partner_id) { // メッセージを取得するSQL $sql="SELECT message_id, send_id, sent_id, message_detail, message_time 
            FROM Message 
            WHERE (send_id = :logged_in_user_id AND sent_id = :partner_id)
               OR (send_id = :partner_id AND sent_id = :logged_in_user_id)
            ORDER BY message_time ASC" ; $stmt=$pdo->prepare($sql);
