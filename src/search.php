    ...
    <table class="user-table">
        ...
        if (!empty($user_data)) {
            foreach ($user_data as $data) {
                echo '<tr>';
                echo '<td class="tag"><a href="javascript:document.form1.submit()"><img src="img/icon/default.jpg" width="40%"height="40%"></td>';
                echo '<input type="hidden" name="user_id" value="',$data['id'],'">';
                echo '<td class="name"><h3>', htmlspecialchars($data['name'], ENT_QUOTES, 'UTF-8'), '</h3></A></td>';
                echo '</tr>';
            }
            $judge = 1;
        }
