    ...
    if ($judge == 1) {
        ...
        if ($pe_user_raw) {
            foreach ($pe_user_raw as $row) {
                echo '<tr><td><img src="img/icon/default.jpg" width="50%" height="50%"></td><td class="name">' . htmlspecialchars($row['user_name']) . '</td></tr>';
            }
        }
        if ($pe_tag_raw) {
            foreach ($pe_tag_raw as $row) {
                echo '<tr><td class="tag">タグ</td><td class="name">' . htmlspecialchars($row['tag_name']) . '</td></tr>';
            }
        }
        echo '</table>';
    } else {
